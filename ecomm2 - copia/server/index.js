...
let usuarioModel = require('./usuario_schema');
...
app.post('/register',(req,res) => {
  ...
  newusuario.nombres =req.body.nombres;
  newusuario.apellidos =req.body.apellidos;
  newusuario.email =req.body.email;
  newusuario.password =req.body.password;
  newusuario.departamento =req.body.departamento;
  newusuario.ciudad =req.body.ciudad;
  newusuario.postalcode =req.body.postalcode;
  newusuario.direccion =req.body.direccion;

  newusuario.save(err => {
      if (err) {
        console.log(err);
        res.send("Error while adding usuario\n" + JSON.stringify(err));
      } else {
